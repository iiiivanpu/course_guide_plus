...
type Props = {
  courseName: string;
  isMobile: string;
  selectAClass: Function;
};
type State = {
  courseName: string | null;
  courseInfo: Array<string> | null;
  loading: boolean;
...
class CourseInfoPage extends Component<Props, State> {
  state = {
    courseName: null,
    courseInfo: null,
    loading: true,
  ...
  renderClass() {
    const { courseExists, courseInfo, loading } = this.state;
    let classElements: Array<React.ReactNode> = [];
    if (!courseExists) {
      classElements.push(this.renderNonExistingMessage());
    } else {
      const allSections = courseInfo;
